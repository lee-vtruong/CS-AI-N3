...
def plot_rastrigin_convergence():
    ...
    for D in dimensions:
        ...
        for algo in algorithms:
            key = f"{algo}_D{D}"
            if key in data:
                plt.plot(data[key], label=algo, linewidth=2, color=colors[algo])
        
        plt.xlabel('Iteration', fontsize=12)
        plt.ylabel('Best Fitness Value', fontsize=12)
        plt.title(f'Convergence Curves - Rastrigin Function (D={D})', fontsize=14, fontweight='bold')
        ...
        output_file = os.path.join(vis_dir, f'convergence_rastrigin_D{D}.png')
        plt.tight_layout()
...
def plot_knapsack_convergence():
    ...
    for n in n_items_list:
        ...
        plt.title(f'Convergence Curves - Knapsack Problem (N={n} items)', fontsize=14, fontweight='bold')
        plt.legend(loc='lower right', fontsize=10)
        plt.grid(True, alpha=0.3)
        
        # Save figure
        vis_dir = os.path.dirname(os.path.abspath(__file__))
        output_file = os.path.join(vis_dir, f'convergence_knapsack_N{n}.png')
        plt.tight_layout()
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        print(f"✓ Saved: {output_file}")
        plt.close()
